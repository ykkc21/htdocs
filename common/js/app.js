$(() => {
  ...
  const Map = {
    ...
    enlargement: function (size) {
      if (size > 200) {
        ...
        $(".img").css({
          width: `${size}%`,
          height: `${size}% ,position": "absolute", "top": "50%", "left": "50%", "transform": "translate(-50%,-50%)`,
        });
        Map.num = 2;
        Map.LV = 10;
  ...
  const list_menu = {
        ...
        list_menu.menus.forEach((item) => {
          if (item.name == name) {
            const image = `<div class="checkBread">
                            <img class="image" src="/breadmenu/${item.image}">
                            <div>메뉴이름:${item.menuname}</div>
                            <div>설명:${item.etc}</div>
                            <div>가격:${item.price}</div>
                            <div>보관방법:${item.keep}</div>
                            <div>open:${item.time}</div>
                            </div>`;
    ...
    SelectItem: function (idx) {
      const id = list_menu.menus.find((e, i) => {
        return e.id == idx;
      });
      return id;
    },
    ItemOverlap: function (idx) {
      const id = list_menu.keep.find((e, i) => {
        return e.id == idx;
      });
      return id;
    },
    KeepItemDraw: function () {
      const array = list_menu.keep.filter((e, i) => {
        console.log(e, i);
      });
    },
  };
